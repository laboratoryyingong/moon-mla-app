#!/usr/bin/env python3
"""
MLA Statistics API - /report/5 NLRS Livestock Indicators Fetcher

Australian National Livestock Reporting Service (NLRS) Livestock Indicators
on an Indicator level basis. Updated daily at 12am AEST.
...
def build_url(from_date: str, to_date: str, indicator_id: int, page: int) -> str:
    params = [
        ("fromDate",    from_date),
        ("toDate",      to_date),
...
def fetch_all(
    ...
    """
    Paginate through all /report/5 results with adaptive rate control (AIMD).
    Iterates over indicator_ids and year chunks separately (API only accepts
    one indicatorID per request and returns HTTP 500 for cross-year queries).

    progress_callback(info: dict) — optional hook for UI integration.
      info["stage"] is one of:
        "category_start" | "page_done" | "waiting" | "warning" | "rate_limited" | "complete"
    ...
    if len(chunks) > 1:
        ...
        for idx, (chunk_from, chunk_to) in enumerate(chunks):
            ...
            try:
                combined.extend(fetch_all(chunk_from, chunk_to, indicator_ids, contact_email, progress_callback))
    ...
    # Iterate indicators one at a time (API only accepts one per request)
    if len(indicator_ids) > 1:
        ...
        for idx, ind_id in enumerate(indicator_ids):
            ...
            combined.extend(fetch_all(from_date, to_date, [ind_id], contact_email, progress_callback))
        return combined

    # Single indicator, single year — paginate
    ...
    while True:
        url = build_url(from_date, to_date, indicator_id, page)
        t0 = time.time()
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Fetch MLA /report/5 NLRS Livestock Indicators data"
    )
...
def main() -> None:
    ...
    if args.list_indicators:
        print("Valid indicators for /report/5:")
    ...
    print("=" * 60)
    print("MLA Statistics API — /report/5 NLRS Livestock Indicators")
    print("=" * 60)
    ...
    try:
        rows = fetch_all(args.from_date, args.to_date, args.indicators, args.email)
